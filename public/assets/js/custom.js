...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let site_url = [root + "/", root + "/roi"];


    if (site_url.includes(current_url)) {
        pageScroll();

        const containEl = document.getElementById("contain");

        containEl.addEventListener("mouseover", function () {
            clearTimeout(my_time);
        });

        containEl.addEventListener("mouseout", function () {
            pageScroll();
        });
...
function getWidthHeader(id_header, id_scroll) {
    const scrollTable = document.getElementById(id_scroll);
    const headerTable = document.getElementById(id_header);
    const firstRow = scrollTable.querySelector("tr:nth-child(1)");

    let colCount = 0;
    const tds = firstRow.querySelectorAll("td");
    tds.forEach(function (td) {
    if (td.hasAttribute("colspan")) {
        colCount += parseInt(td.getAttribute("colspan"));
    } else {
        colCount++;
    }
    });
    ...
    const scrollTd = scrollTable.querySelector(`tbody > tr:first-child > td:nth-child(${i + 1})`);
    const width = scrollTd.offsetWidth;

    const headerTh = headerTable.querySelector(`thead th:nth-child(${i + 1})`);
    if (headerTh) {
        headerTh.style.width = width + "px";
    }
    }
}
